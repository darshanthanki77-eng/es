...
exports.getDashboardStats = async (req, res) => {
    try {
        ...
        // 1. Total Products
        // Database uses String for seller_id in products and orders for this user
        const sellerIdStr = String(seller.id);

        const totalProducts = await SellerProduct.countDocuments({ seller_id: sellerIdStr });

        // 2. Total Orders (assigned to this seller)
        const totalOrders = await Order.countDocuments({ seller_id: sellerIdStr });

        // 3. Total Sales (Confirmed/Completed orders volume)
        // Adjusting status filter to include 'shipped' and 'processing' as per DB observation
        // Casting string order_total to double for sum
        const salesResult = await Order.aggregate([
            { $match: { seller_id: sellerIdStr, status: { $regex: 'processing|shipped|completed|delivered', $options: 'i' } } },
        ...
        const guaranteeResult = await GuaranteeMoney.aggregate([
            { $match: { seller_id: seller.id, status: 1 } },
        ...
        const myProductLinks = await SellerProduct.find({ seller_id: sellerIdStr });
...
exports.updateTransactionPassword = async (req, res) => {
    try {
        ...
        seller.trans_password = newPassword;
        await seller.save();
