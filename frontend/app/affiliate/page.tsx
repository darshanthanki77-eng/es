...
export default function AffiliatePage() {
    ...
    const referralLink = "https://smartseller.pro/ref/demo123";
    ...
    return (
        <Shell>
            <div className="space-y-10 pb-20 max-w-[1600px] mx-auto">
                ...
                <div className="relative p-10 lg:p-16 rounded-[3rem] bg-slate-900 overflow-hidden group">
                    ...
                    <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="grid grid-cols-2 gap-6 relative">
                            {[
                                { label: 'Total Earned', value: '$4,280', icon: DollarSign, color: 'text-success-400' },
                                { label: 'Active Referrals', value: '142', icon: Users, color: 'text-primary-400' },
                                { label: 'Conversion Rate', value: '18.5%', icon: TrendingUp, color: 'text-purple-400' },
                                { label: 'Pending Payout', value: '$240', icon: Gift, color: 'text-yellow-400' },
                            ].map((stat, i) => (
                ...
                {/* How it works */}
                <div className="space-y-8">
                    <h3 className="text-3xl font-black text-gray-900 text-center">Three Simple Steps</h3>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {[
                                    { name: 'Alex Thompson', date: '2 hours ago', status: 'Active', amount: '$12.50' },
                                    { name: 'Sarah Miller', date: '5 hours ago', status: 'Active', amount: '$45.00' },
                                    { name: 'Mike Ross', date: 'Yesterday', status: 'Pending', amount: '$0.00' },
                                    { name: 'Emma Watson', date: '2 days ago', status: 'Active', amount: '$15.20' },
                                ].map((ref, i) => (
                                    <tr key={i} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-8 py-6 font-bold text-gray-900 flex items-center gap-3">
                                            <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-xs font-black text-gray-400 group-hover:bg-primary-100 group-hover:text-primary-600 transition-colors">
                                                {ref.name.split(' ').map(n => n[0]).join('')}
                                            </div>
                                            {ref.name}
                                        </td>
                                        <td className="px-8 py-6 text-gray-500 font-medium">{ref.date}</td>
                                        <td className="px-8 py-6">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${ref.status === 'Active' ? 'bg-success-50 text-success-600' : 'bg-yellow-50 text-yellow-600'
                                                }`}>
                                                {ref.status}
                                            </span>
                                        </td>
                                        <td className="px-8 py-6 text-right font-black text-gray-900">{ref.amount}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
