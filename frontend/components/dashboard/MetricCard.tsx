        ...
        <div className="glass-card p-5 md:p-6 group cursor-default hover:border-primary-300 transition-all duration-500 relative overflow-hidden h-full flex flex-col justify-between">
            ...
            <div className="flex items-start justify-between relative z-10 gap-2">
                <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] truncate">{title}</p>
                    <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-gray-900 tracking-tight transition-transform group-hover:translate-x-1 duration-500 truncate mt-1">
                        {value}
                    </h3>
                    {subtitle && (
                        <p className="text-[10px] md:text-xs font-bold text-gray-500 flex items-center gap-1 mt-1 truncate">
            ...
            <div className="mt-4 md:mt-6 flex items-end justify-between relative z-10">
                {change !== undefined ? (
                    <div className="flex items-center gap-2">
                        <div className={`flex items-center gap-1 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-[11px] font-black ${isPositive ? 'bg-success-100 text-success-700' : 'bg-danger-100 text-danger-700'
                            }`}>
                            {isPositive ? (
                        ...
                        <span className="text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none hidden sm:block">Trend</span>
                    </div>
                ) : (
